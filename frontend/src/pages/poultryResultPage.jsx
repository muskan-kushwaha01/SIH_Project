...
const PoultryRiskResultPage = () => {
  ...
  useEffect(() => {
    const fetchResult = async () => {
      ...
      try {
        // Fetch poultry result from backend
        ...
        if (!response.ok) {
          ...
          if (response.status === 404) {
            alert("No poultry risk analysis found. Please complete the risk analysis first.");
          } else {
        ...
        setResult(data.prediction); // Ensure your backend returns 'prediction'
        console.log("Loaded poultry result:", data.prediction);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-lg">Loading your poultry risk analysis...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 px-4 py-8">
      <h1 className="text-3xl md:text-4xl font-bold text-gray-800 mb-6 text-center">
        Your Poultry Farm Risk Analysis Result
      </h1>
